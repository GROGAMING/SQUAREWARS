// Core AI implementation extracted from legacy ai.js
// Public API: chooseComputerMoveCore({ grid, blockedCells, aiDifficulty }) -> number
// This contains the full strong AI: config, helpers, evaluation, search, and difficulty entry points.

import { ROWS, COLS, PLAYER, DIRECTIONS, AI as AI_IN } from "./constants.js";

export function chooseComputerMoveCore({ grid, blockedCells, aiDifficulty }) {
  ...
  for (const col of order) {
    ...
    if (isMax) {
      ...
      if (alpha >= beta) {
        ...
        if (history) history[col] = (history[col] | 0) + depth * depth;
        break;
      }
    } else {
      if (val < bestVal) {
...
function getBestMoveAdvanced(grid, blocked, hashBase, AI) {
  ...
  for (let col of candidates) {
    ...
    undoMove(grid, blocked, move, hashBase, true);
    const pref = AI.ADVANCED_PICK_SPLITS || { BEST: 0.85, SECOND: 0.97 };
    const bias = Math.random() < pref.SECOND ? 0.95 : 1;
    moves.push({ col, score: score * bias });
  }
  if (!moves.length) return -1;
  moves.sort((a, b) => b.score - a.score);
  return moves[0].col;
...
function getBestMoveImpossible(grid, blocked, hashBase, AI) {
  const depth = effectiveDepth(AI.IMPOSSIBLE_DEPTH, grid, blocked, AI.IMPOSSIBLE_DEPTH);
  ...
  for (let col = 0; col < COLS; col++) {
    ...
    const row = getDropRow(grid, blocked, col);
    grid[row][col] = PLAYER.BLUE;
    const win = checkForWinSimulation(grid, blocked, row, col, PLAYER.BLUE);
    grid[row][col] = 0;
    if (win) return col;
  }

  // B) Zero-tolerance: never hand Red an immediate close
  const safe = [];
  const centerOrder = [];
  for (let d = 0; d < COLS; d++) {
    const L = Math.floor(COLS / 2) - d;
    const R = Math.floor(COLS / 2) + d;
    if (L >= 0) centerOrder.push(L);
    if (R < COLS) centerOrder.push(R);
  }
  for (const col of centerOrder) {
    if (!canDropInColumn(grid, blocked, col)) continue;
    const after = getOppImmediateWinsAfterOurMove(grid, blocked, col, hashBase);
    if (after.count === 0) safe.push(col);
  }
  if (safe.length) return safe[0];

  // C) Proactive block of Red one-move fork setup
  const preBlock = findBlockForOpponentOneMoveFork(grid, blocked, hashBase);
  if (preBlock !== -1) return preBlock;

  // D) Search with budgets; keep PV move and scores
  const nodeBudget = { count: 140_000 };
  const start = now();
  const timeBudget = 32;
  ...
  for (let depth0 = Math.max(2, depth - 1); depth0 <= depth + 1; depth0++) {
    const candidates = getCandidateMovesOrdered(grid, blocked, 9, pvMove, history);
    let localBest = -1;
    ...
    for (const col of candidates) {
      ...
      undoMove(grid, blocked, move, hashBase, true);
      const centerBonus = Math.max(0, 5 - Math.abs(col - COLS / 2)) * 4;
      const total = score + centerBonus - softPenaltyAfterMove(grid, blocked, col, hashBase);
      rootInfo.push({ c: col, score: total, wins: getOppImmediateWinsAfterOurMove(grid, blocked, col, hashBase), fork: createsOpponentForkAfterOurMove(grid, blocked, col, hashBase) });
      if (total > bestScore) {
        bestScore = total;
        localBest = col;
      }
